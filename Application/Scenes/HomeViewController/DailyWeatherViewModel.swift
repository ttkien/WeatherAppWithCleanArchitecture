...
class DailyWeatherViewModel : ViewModelType {

    enum State {
        case `default`
        case empty
        case loading
        case success(models: WeekWeatherResult)
    ...
    func getWeather(locationFilters: LocationFilters) -> Observable<WeekWeatherResult?> {
        return self.useCase.getWeekWeatherResult(location: locationFilters)
    }
}
